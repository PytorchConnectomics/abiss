...
int main(int argc, char** argv) {
    ...
    try {
        ...
        size_t p;
        // No voxel cap at 2^31: uint32 bounds segment ids, not voxel positions.
        check(watershed_size_valid(limit>>1,1,1,p), "past old 2^31 voxel cap");
        check(watershed_size_valid(limit+1,1,1,p), "past 2^32 voxels");
        const size_t high64 = size_t(1)<<63;
        check(watershed_size_valid(high64-1,1,1,p), "PTRDIFF_MAX");
        check(!watershed_size_valid(high64,1,1,p), "PTRDIFF_MAX+1");
        check(!watershed_size_valid(SIZE_MAX,2,2,p), "product overflow");
        queues<uint32_t>();
        queues<uint64_t>();
