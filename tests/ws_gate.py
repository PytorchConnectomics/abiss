    ...
    def sizes(self):
        ...
        for width, high in (("ws", 2**31), ("ws64", 2**63)):
            for label, shape in [
                ...
                # Only the voxel product must fit ptrdiff_t; each case then exits 2
                # on the 1-voxel-thick dimensions if it passed the size check.
                fits = shape[0] * shape[1] * shape[2] <= 2**63 - 1
                require(("Chunk size check passed:" in log) == fits, log)
